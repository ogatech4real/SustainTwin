...
Physics-aware process model — generates the dynamic mechanical, electrical and thermal state.
Bidirectional communication — exchanges telemetry and experiment commands through MQTT.
Experiment orchestration — Node-RED controls fault class, severity, timing, replicate and run status.
Analytical evaluation — qualified data are used for physics-response analysis, external behavioural comparison and experiment-level machine-learning evaluation.
---
...
Continuous five-class monitoring — `F0–F4` across baseline, fault-active and recovery periods.
Healthy--fault detection — binary discrimination between healthy and active-fault states.
Fault-active four-class classification — discrimination among `F1–F4` during the active-fault interval.
...
<p align="center"><em>Pooled out-of-fold confusion matrix for the highest-performing continuous five-class model under experiment-level evaluation.</em></p>
The fault-active score reflects strong separation among the programmed PhysDiagTwin fault states once their effects are established. It should not be interpreted as near-perfect industrial diagnosis.
---
...
<p align="center"><em>Comparison of electromechanical relationships empirically estimable in both PhysDiagTwin and the independent industrial historian.</em></p>
Relationship	PhysDiagTwin	Industrial historian
...
experiment-level diagnostic performance within PhysDiagTwin; and
...
PhysDiagTwin therefore uses the complete experimental run as the machine-learning partition boundary:
```text
Run A ─┐
Run B ─┼── TRAIN
Run C ─┘
...
---
Manuscript
PhysDiagTwin: A Physics-Aware Digital Twin Framework for Controlled Fault-Data Generation and Diagnostic Evaluation
...
---
Citation
If you use PhysDiagTwin, its methodology, code or archived dataset, please cite the associated manuscript and research artifact.
A machine-readable citation template is included in `CITATION.cff`.
